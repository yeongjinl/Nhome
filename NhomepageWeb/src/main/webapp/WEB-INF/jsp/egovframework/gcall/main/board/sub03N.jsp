		...
		<script>
			$(function() {
				...
				$("#searchBtn").on("click", function() {
					...
					window.location.href = "<c:url value='/board/promotion.do'/>" + "?curPage=1&scType=" + $("#searchTypeSelect").val() + "&scText=" + $("#searchInput").val();
				});
			});
		</script>
	</head>
			   ...
			   <div class='container'>
						...
						<div class="sub-depth-2">
								<ul> 
									...
									<li class="active"><a href="<c:url value='/board/promotion.do'/>" title="홍보자료 선택됨">홍보자료</a></li>
								</ul>  
						</div>    
                        	...
                        	<c:choose>
							 ...
							 <div id='boardList'>
								...
								<table class='table text-nowrap'> 
									<caption><span class="blind">홍보자료 목록</span></caption>
									<colgroup><col style='width:10%;'><col style='width:56%;'><col style='width:10%;'><col style='width:14%;'><col style='width:10%;'></colgroup>
										...
										<c:forEach var="obj" items="${promotionList}" varStatus="status">
										<tr>
<!-- 											<td class="num"> -->
<%-- 												<c:choose> --%>
...
<!-- 											</td> -->
											<td class="left subject">
												...
												<a tabindex="${obj.BOARD_NUM}39" onmouseover="fn_mouseOver('${obj.BOARD_NUM}39','${obj.BOARD_TITLE}')" onmouseout="fn_mouseOut('${obj.BOARD_NUM}39')" href="<c:url value='/board/promotionView.do?num=${obj.BOARD_NUM}&curPage=${paginationDTO.curPage}&scType=${scType}&scText=${scText}'/>">
								...
								<ul>
									<li>
									     <c:if test="${paginationDTO.curRange ne 1 }">
											<a tabindex="42" onmouseover="fn_mouseOver('42','처음')" onmouseout="fn_mouseOut('42')" href="<c:url value='/board/promotion.do?curPage=1&scType=${scType}&scText=${scText}'/>" class='first'>처음</a>
										</c:if>
									</li>
									<li>	
										 <c:if test="${paginationDTO.curPage ne 1}">
										  <a tabindex="43" onmouseover="fn_mouseOver('43','이전')" onmouseout="fn_mouseOut('43')" href="<c:url value='/board/promotion.do?curPage=${paginationDTO.prevPage}&scType=${scType}&scText=${scText}'/>" class='prev'>이전</a>
										</c:if>
									</li>
									<li>
										<c:forEach var="pageNum" begin="${paginationDTO.startPage}" end="${paginationDTO.endPage}">
											<c:choose>
												<c:when test="${pageNum eq paginationDTO.curPage}">
													<strong><a href="<c:url value='/board/promotion.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>" class='active' title="선택됨">${fn:escapeXml(pageNum)}</a></strong>
												</c:when>
												<c:otherwise>
													<a href="<c:url value='/board/promotion.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>">${fn:escapeXml(pageNum)}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</li>
									<li>
										<c:if test="${paginationDTO.curPage ne paginationDTO.totalPage && paginationDTO.totalPage > 0}">
											<a tabindex="44" onmouseover="fn_mouseOver('44','다음')" onmouseout="fn_mouseOut('44')" href="<c:url value='/board/promotion.do?curPage=${paginationDTO.nextPage}&scType=${scType}&scText=${scText}'/>" class='next'>다음</a>
										</c:if>
									</li>
									<li>
										 <c:if test="${paginationDTO.curRange ne paginationDTO.totalRange && paginationDTO.totalRange > 0}">
											<a tabindex="45" onmouseover="fn_mouseOver('45','마지막')" onmouseout="fn_mouseOut('45')" href="<c:url value='/board/promotion.do?curPage=${paginationDTO.totalPage}&scType=${scType}&scText=${scText}'/>" class='last'>마지막</a>
										</c:if>
									</li>
								</ul>
							</nav>
