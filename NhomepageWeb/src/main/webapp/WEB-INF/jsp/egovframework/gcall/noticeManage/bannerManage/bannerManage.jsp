		...
		success : function(data){
			...
			$.each(data, function(key,value){
				...
				if(value.ALARM_REAL_PATH == "") {
					...
					contents+='<td><img src="https://www.110.go.kr'+value.ALARM_REAL_PATH+'" alt="'+value.ALARM_ID+'" class="h100"></td>';
				}
...
$(document).on("change","#selUse",function(){
	$.ajax({
		...
		success : function(data){
			var contents = '';
			$.each(data, function(key,value){
 				contents+='<tr onclick="javascript:popupDetail(\''+value.ALARM_ID+'\')" style="cursor: pointer;">';
				contents+='<td>'+value.ALARM_ID+'</td>';
				contents+='<td>'+value.ALARM_TITLE+'</td>';
				contents+='<td><img src="https://www.110.go.kr'+value.ALARM_REAL_PATH+'" alt="'+value.ALARM_ID+'"></td>';
				contents+='<td>'+value.ALARM_LINK_URL+'</td>';
				if(value.ALARM_TARGET==null||value.ALARM_TARGET==undefined||value.ALARM_TARGET=='') {contents+='<td>-</td>';} else {contents+='<td>'+value.ALARM_TARGET+'</td>';}
				if(value.ALARM_USE=="1") {contents+='<td>O</td>';} else {contents+='<td>X</td>';}
				if(value.REG_ID=="admin") {contents+='<td>관리자</td>';} else {contents+='<td>'+value.REG_ID+'</td>';}
...
<body>
	...
	<table class='layout'>
		...
		<tr>
			<td class='leftMenuArea'>
				<!-- LeftMenu Area -->
				<ul class='leftMenu' id="noticeLeftMenu">
				</ul>
				<!--/ LeftMenu Area -->
			</td>
			<td class='contentArea'>
				...
				<div class='page-title'>
					<div class='title'>배너 관리</div>
					<div class='navi'><i class='fa fa-home'></i> <i class='fa fa-chevron-right'></i> 공지관리 <i class='fa fa-chevron-right'></i> <span class='now'>배너 관리</span></div>
				</div>
				<div class="panel panel-default writeBox">
				  ...
				  <div class="panel-body">
			  			...
			  			<form  action="/popup/regProc.do" id="codetypeForm" method="post" enctype="multipart/form-data">
			  				<div class='hidden'>
			  					<input type='hidden' id="CONFID" name="CONFID" value="92"/>
			  					<input type='hidden' id="UPDATE" name="UPDATE" value="insert"/>
			  					<input type='hidden' id="ALARM_ID" name="ALARM_ID" value=""/>
			  					<input type='hidden' id="ALARM_TYPE" name="ALARM_TYPE" value="1"/>
		  					</div>
					  		<table class='table table-bordered'>
					  			<tbody>
					  				<tr>
					  					<th><label for='ALARM_TITLE'>* 배너제목</label></th>
					  						...
					  						<table>
					  							<tbody>
					  								...
					  								<tr>
						  								...
						  								<td colspan=3>
						  									<select class='form-control' id='ALARM_TARGET' name='ALARM_TARGET'>
													    		<option value='0' disabled="disabled" selected>선택</option>
					  								...
					  								<tr>
						  								...
						  								<td><input type='text' id='ALARM_REAL_PATH' name='ALARM_REAL_PATH' style='width:auto; pointer-events:none; background-color:#eee; color:#555; opacity:1;' class='form-control'></td>
					  								</tr>
					  								<tr>
						  								...
						  								<td colspan=3>
												            <div class="form-group" style="margin-bottom: 0px;">
													            <label for="fileUp"  class="fileDrop" title="사진추가" style="margin-bottom: 0px;">
													                ...
													                <input type="file" id="fileUp" title="사진추가" style="display:none"/>
												            </div>
						  								</td>
					  								</tr>
					  							</tbody>
	...
	<script id="fileTemplate" type="text/x-handlebars-template">
        <img style="width:100%;" src="https://www.110.go.kr{{{imgSrc}}}" id="regImg2" name="regImg" class="img-responsive" onload='regButton()' alt="첨부파일">
