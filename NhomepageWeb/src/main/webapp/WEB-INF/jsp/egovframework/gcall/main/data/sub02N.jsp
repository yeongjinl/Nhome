		...
		<script>
			$(function() {
				...
				$("#searchBtn").on("click", function() {
					...
					window.location.href = "<c:url value='/data/story.do'/>" + "?curPage=1&scType=" + $("#searchTypeSelect").val() + "&scText=" + $("#searchInput").val();
				});
			});
		</script>
    </head>
               ...
               <div class='container'>
						<div class="sub-visual subv-1">
							...
							<div class="sub-depth-1">  
								<ul> 
									...
									<li class="active" tabindex="27" onmouseover="fn_mouseOver('27','상담사연')" onmouseout="fn_mouseOut('27')"><a href="<c:url value='/data/story.do'/>" title="상담사연 선택됨">상담사연</a></li>
						...
						<div class="sub-depth-2">
								<ul> 
									...
									<li class="active"><a href="<c:url value='/data/story.do'/>">상담사연</a></li>
							...
							<c:choose>
							 ...
							 <div id='boardList'>
								...
								<table class='table text-nowrap'> 
									<caption><span class="blind">상담사연 목록</span></caption>
									...
									<c:forEach var="obj" items="${storyList}" varStatus="status">
									<tr>
										 ...
										 <fmt:parseDate value="${obj.BOARD_REG_DATE}" pattern="yyyy-MM-dd" var="boardDate"/>
                                            ...
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                       						  ...
                       						  <a tabindex="${obj.BOARD_NUM}39" onmouseover="fn_mouseOver('${obj.BOARD_NUM}39','${obj.BOARD_TITLE}')" onmouseout="fn_mouseOut('${obj.BOARD_NUM}39')" href="<c:url value='/data/storyView.do?num=${obj.BOARD_NUM}&curPage=${paginationDTO.curPage}&scType=${scType}&scText=${scText}'/>">${fn:escapeXml(obj.BOARD_TITLE)}</a>
										</td>
										<td class="file">
										...
										<td tabindex="${obj.BOARD_NUM}40" onmouseover="fn_mouseOver('${obj.BOARD_NUM}40','${obj.BOARD_REG_DATE}')" onmouseout="fn_mouseOut('${obj.BOARD_NUM}40')" class="date">${fn:escapeXml(obj.BOARD_REG_DATE)}</td>
										<td tabindex="${obj.BOARD_NUM}41" onmouseover="fn_mouseOver('${obj.BOARD_NUM}41','${obj.BOARD_VIEW}')" onmouseout="fn_mouseOut('${obj.BOARD_NUM}41')" class="comment">${fn:escapeXml(obj.BOARD_VIEW)}</td>
									</tr>
									</c:forEach>
								...
								<ul>
									<li>
									     <c:if test="${paginationDTO.curRange ne 1 }">
											<a tabindex="42" onmouseover="fn_mouseOver('42','처음')" onmouseout="fn_mouseOut('42')" href="<c:url value='/data/story.do?curPage=1&scType=${scType}&scText=${scText}'/>" class='first'>처음</a>
										</c:if>
									</li>
									<li>
										
										 <c:if test="${paginationDTO.curPage ne 1}">
										  <a tabindex="43" onmouseover="fn_mouseOver('43','이전')" onmouseout="fn_mouseOut('43')" href="<c:url value='/data/story.do?curPage=${paginationDTO.prevPage}&scType=${scType}&scText=${scText}'/>" class='prev'>이전</a>
										</c:if>
									</li>
									<li>
										<c:forEach var="pageNum" begin="${paginationDTO.startPage}" end="${paginationDTO.endPage}">
											<c:choose>
												<c:when test="${pageNum eq paginationDTO.curPage}">
													<strong><a href="<c:url value='/data/story.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>" class='active' title="선택됨">${fn:escapeXml(pageNum)}</a></strong>
												</c:when>
												<c:otherwise>
													<a href="<c:url value='/data/story.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>">${fn:escapeXml(pageNum)}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</li>
									<li>
										<c:if test="${paginationDTO.curPage ne paginationDTO.totalPage && paginationDTO.totalPage > 0}">
											<a tabindex="44" onmouseover="fn_mouseOver('44','다음')" onmouseout="fn_mouseOut('44')" href="<c:url value='/data/story.do?curPage=${paginationDTO.nextPage}&scType=${scType}&scText=${scText}'/>" class='next'>다음</a>
										</c:if>
									</li>
									<li>
										 <c:if test="${paginationDTO.curRange ne paginationDTO.totalRange && paginationDTO.totalRange > 0}">
											<a tabindex="45" onmouseover="fn_mouseOver('45','마지막')" onmouseout="fn_mouseOut('45')" href="<c:url value='/data/story.do?curPage=${paginationDTO.totalPage}&scType=${scType}&scText=${scText}'/>" class='last'>마지막</a>
										</c:if>
									</li>
								</ul>
							</nav>
