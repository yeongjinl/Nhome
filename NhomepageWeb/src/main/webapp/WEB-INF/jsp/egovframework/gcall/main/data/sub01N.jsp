               ...
               <div class='container'>
						<div class="sub-visual subv-1">
							...
							<div class="sub-depth-1">  
								<ul> 
									<li class="active" tabindex="26" onmouseover="fn_mouseOver('26','자주묻는 질문에프에이큐')" onmouseout="fn_mouseOut('26')"><a href="<c:url value='/data/faq.do'/>" title="자주묻는 질문(FAQ) 선택됨">자주묻는 질문(FAQ)</a></li>
						...
						<div class="sub-depth-2">
								<ul> 
									<li class="active"><a href="<c:url value='/data/faq.do'/>" title="자주묻는 질문(FAQ) 선택됨">자주묻는 질문(FAQ)</a></li>
							...
							<!-- //검색 -->
							 ...
							 <div id='boardList'>
								...
								<table class='table text-nowrap'> 
									<caption><span class="blind">자주묻는질문(FAQ) 목록</span></caption>
									<colgroup><col style='width:10%;'><col style='width:46%;'><col style='width:10%;'><col style='width:10%;'><col style='width:14%;'><col style='width:10%;'></colgroup>
										...
										<thead><tr>
											  ...
											  <th tabindex="35" onmouseover="fn_mouseOver('35','구분')" onmouseout="fn_mouseOut('35')" scope="col">구분</th>
											  <th tabindex="36" onmouseover="fn_mouseOver('36','파일')" onmouseout="fn_mouseOut('36')" scope="col">파일</th>
											  <th tabindex="37" onmouseover="fn_mouseOver('37','작성일')" onmouseout="fn_mouseOut('37')" scope="col">작성일</th>
											  <th tabindex="38" onmouseover="fn_mouseOver('38','조회수')" onmouseout="fn_mouseOut('38')" scope="col">조회수</th>
											  </tr>
										</thead>
										<tbody>
									<!-- 게시글 출력 -->
									...
									<tr>
										<td tabindex="${obj.DB_SEQ}39" onmouseover="fn_mouseOver('${obj.DB_SEQ}39','${paginationDTO.totalCount - paginationDTO.pageSize * (paginationDTO.curPage - 1) - listIndex}')" onmouseout="fn_mouseOut('${obj.DB_SEQ}39')" class="num">${paginationDTO.totalCount - paginationDTO.pageSize * (paginationDTO.curPage - 1) - listIndex}</td>
										...
										<td class="left subject">
										 <fmt:parseDate value="${obj.REG_DATE}" pattern="yyyy-MM-dd" var="boardDate"/>
                                            ...
                                            <a tabindex="${obj.DB_SEQ}40" onmouseover="fn_mouseOver('${obj.DB_SEQ}40','${obj.TITLE}')" onmouseout="fn_mouseOut('${obj.DB_SEQ}40')" href="<c:url value='/data/faqView.do?num=${obj.DB_SEQ}&curPage=${paginationDTO.curPage}&scType=${scType}&scText=${scText}'/>">${fn:escapeXml(obj.TITLE)}</a>
										</td>
										<td class="cat">
											<c:choose>
										...
										<td class="file">
											...
											<td tabindex="${obj.DB_SEQ}41" onmouseover="fn_mouseOver('${obj.DB_SEQ}41','${obj.REG_DATE}')" onmouseout="fn_mouseOut('${obj.DB_SEQ}41')" class="date">${fn:escapeXml(obj.REG_DATE)}</td>
										<td tabindex="${obj.DB_SEQ}42" onmouseover="fn_mouseOver('${obj.DB_SEQ}42','${obj.VIEW_CNT}')" onmouseout="fn_mouseOut('${obj.DB_SEQ}42')" class="comment">${fn:escapeXml(obj.VIEW_CNT)}</td>
									</tr>
									</c:forEach>
								...
								<ul>
									<li>
									     <c:if test="${paginationDTO.curRange ne 1 }">
											<a tabindex="42" onmouseover="fn_mouseOver('42','처음')" onmouseout="fn_mouseOut('42')" href="<c:url value='/data/faq.do?curPage=1&scType=${scType}&scText=${scText}'/>" class='first'>처음</a>
										</c:if>
									</li>
									<li>
										
										 <c:if test="${paginationDTO.curPage ne 1}">
										  <a tabindex="43" onmouseover="fn_mouseOver('43','이전')" onmouseout="fn_mouseOut('43')" href="<c:url value='/data/faq.do?curPage=${paginationDTO.prevPage}&scType=${scType}&scText=${scText}'/>" class='prev'>이전</a>
										</c:if>
									</li>
									<li>
										<c:forEach var="pageNum" begin="${paginationDTO.startPage}" end="${paginationDTO.endPage}">
											<c:choose>
												<c:when test="${pageNum eq paginationDTO.curPage}">
													<strong><a href="<c:url value='/data/faq.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>" class='active' title="선택됨">${fn:escapeXml(pageNum)}</a></strong>
												</c:when>
												<c:otherwise>
													<a href="<c:url value='/data/faq.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>">${fn:escapeXml(pageNum)}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</li>
									<li>
										<c:if test="${paginationDTO.curPage ne paginationDTO.totalPage && paginationDTO.totalPage > 0}">
											<a tabindex="44" onmouseover="fn_mouseOver('44','다음')" onmouseout="fn_mouseOut('44')" href="<c:url value='/data/faq.do?curPage=${paginationDTO.nextPage}&scType=${scType}&scText=${scText}'/>" class='next'>다음</a>
										</c:if>
									</li>
									<li>
										 <c:if test="${paginationDTO.curRange ne paginationDTO.totalRange && paginationDTO.totalRange > 0}">
											<a tabindex="45" onmouseover="fn_mouseOver('45','마지막')" onmouseout="fn_mouseOut('45')" href="<c:url value='/data/faq.do?curPage=${paginationDTO.totalPage}&scType=${scType}&scText=${scText}'/>" class='last'>마지막</a>
										</c:if>
									</li>
								</ul>
							</nav>
