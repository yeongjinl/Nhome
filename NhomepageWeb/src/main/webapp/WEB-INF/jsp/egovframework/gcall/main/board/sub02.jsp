                ...
                <div class='contentArea' id='contentArea'>
                    <div class='limitWrap'>
                        ...
                        <table class='table table-bordered boardList'>
                            <caption class='blind'>보도자료 목록</caption>
                            <thead>
                                <tr>
                                    <th class='num'>번호</th>
                                    <th class='ttl'>제목</th>
                                ...
                                <c:forEach var="obj" items="${newsList}" varStatus="status">
                                    <tr>
                                        ...
                                        <td class='ttl'>
                                            ...
                                            <a href="<c:url value='/board/newsView.do?num=${obj.BOARD_NUM}&curPage=${paginationDTO.curPage}&scType=${scType}&scText=${scText}'/>">${obj.BOARD_TITLE}</a>
                                        </td>
                                        <td>
                        ...
                        <c:if test="${newsList.size() > 0}">
                            <div class='pagination'>
                                <c:if test="${paginationDTO.curRange ne 1 }">
                                    <a href="<c:url value='/board/news.do?curPage=1&scType=${scType}&scText=${scText}'/>" class='move'>처음</a>
                                </c:if>
                                
                                <c:if test="${paginationDTO.curPage ne 1}">
                                <a href="<c:url value='/board/news.do?curPage=${paginationDTO.prevPage}&scType=${scType}&scText=${scText}'/>" class='move'>이전</a>
                                </c:if>
                                ...
                                <c:forEach var="pageNum" begin="${paginationDTO.startPage}" end="${paginationDTO.endPage}">
                                    <c:choose>
                                        <c:when test="${pageNum eq paginationDTO.curPage}">
                                            <a href="<c:url value='/board/news.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>" class='active' title="선택됨">${pageNum}</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="<c:url value='/board/news.do?curPage=${pageNum}&scType=${scType}&scText=${scText}'/>">${pageNum}</a>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                                
                                <c:if test="${paginationDTO.curPage ne paginationDTO.totalPage && paginationDTO.totalPage > 0}">
                                    <a href="<c:url value='/board/news.do?curPage=${paginationDTO.nextPage}&scType=${scType}&scText=${scText}'/>" class='move'>다음</a>
                                </c:if>
                                
                                <c:if test="${paginationDTO.curRange ne paginationDTO.totalRange && paginationDTO.totalRange > 0}">
                                    <a href="<c:url value='/board/news.do?curPage=${paginationDTO.totalPage}&scType=${scType}&scText=${scText}'/>" class='move'>마지막</a>
                                </c:if>
                            </div>
                        </c:if>
                    </div>
