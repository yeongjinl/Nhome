				...
				<td class='leftMenuArea'>
					...
					<td class='contentArea'>
						...
						<div class='page-title'>
							...
							<div class='navi'><i class='fa fa-home'></i> <i class='fa fa-chevron-right'></i> 게시물관리 <i class='fa fa-chevron-right'></i> <span class='now'>공지사항 관리</span></div>
						</div>
						...
						<div class="panel panel-default searchBox">
						  <div class="panel-body">
						    	...
						    	<c:if test="${NOTICE_AUTH eq '1' or sUserType eq '0'}">
    								<button type='button' class='btn btn-primary btn-lg' onclick='location.href="/notice/reg.do"'>등록하기</button>
								</c:if>
						  </div>
						</div>
						<div class="panel panel-default listBox">
						  <div class="panel-body">
						  	...
						  	<table class='table table-bordered table-list' summary="공지사항관리 목록을 나타냅니다.">
						 		<caption style="display:none">공지사항관리 게시판</caption>
						  		...
						  		<% for(int i = 0 ; i < hList.size(); i++){ %>
						  			<tr class="selected">
						  				...
						  				<td scope="col"><a href="/notice/detail.do?boardNum=<%=hList.get(i).get("BOARD_NUM") %>&confId=<%=hList.get(i).get("CONF_ID") %>"><%=hList.get(i).get("TITLE") %></a></td>
							  ...
							  <ul class="pagination">
							    ...
							    <%} else { %>
									<li class='box'>
								    	<a  href="/notice/list.do?selSearch=<%=pDTO.getSelSearch()%>&searchWord=<%=java.net.URLEncoder.encode(pDTO.getSearchWord(),"UTF-8")%>" aria-label="Previous"><span aria-hidden="true"><i class='fa fa-caret-left'></i><i class='fa fa-caret-left'></i></span></a>
								    </li>
								    <li class='box'>
								     	<a href="/notice/list.do?curPage=<%=pDTO.getStartPage()-5%>&selSearch=<%=pDTO.getSelSearch()%>&searchWord=<%=java.net.URLEncoder.encode(pDTO.getSearchWord(),"UTF-8")%>" aria-label="Previous"><span aria-hidden="true"><i class='fa fa-caret-left'></i></span></a>
								    </li>
							     <%} %>

							    <% for(int i =pDTO.getStartPage() ; i<=pDTO.getEndPage();i++ ){ %>
							   		<li><a href="/notice/list.do?curPage=<%=i%>&selSearch=<%=pDTO.getSelSearch()%>&searchWord=<%=java.net.URLEncoder.encode(pDTO.getSearchWord(),"UTF-8")%>" id="pageNum<%=i%>" class="pageNum"><%=i%></a></li>
								<%} %>
								<%if(pDTO.getTotalPage()==pDTO.getEndPage()) {%>
								<%} else{ %>
									<li class='box'>
								    	<a href="/notice/list.do?curPage=<%=pDTO.getEndPage()+1%>&selSearch=<%=pDTO.getSelSearch()%>&searchWord=<%=java.net.URLEncoder.encode(pDTO.getSearchWord(),"UTF-8")%>" aria-label="Previous"><span aria-hidden="true"><i class='fa fa-caret-right'></i></span></a>
								    </li>
								    <li class='box'>
								    	<a href="/notice/list.do?curPage=<%=pDTO.getTotalPage()%>&selSearch=<%=pDTO.getSelSearch()%>&searchWord=<%=java.net.URLEncoder.encode(pDTO.getSearchWord(),"UTF-8")%>" aria-label="Previous"><span aria-hidden="true"><i class='fa fa-caret-right'></i><i class='fa fa-caret-right'></i></span></a>
								    </li>
								<%} %>
							  </ul>
							</nav>
	...
	<script>
		$(function(){
			...
			$("#searchList").click(function(){
				...
				location.href="/notice/list.do?selSearch="+$('#selSearch option:selected').val()+"&searchWord="+encodeURI($('#searchWord').val());
			})
		})
	</script>
</body>
