...
<body>
	<table class='layout' summary="상담사연 상세 ">
		<%@include file="/WEB-INF/jsp/egovframework/gcall/index/top.jsp" %>
		<caption style="display:none">상담 사연 상세</caption>
		<tbody>
			<tr>
				<td scope="col" class='leftMenuArea'>
					...
					<td scope="col" class='contentArea'>
						...
						<div class='page-title'>
							...
							<div class='navi'><i class='fa fa-home'></i> <i class='fa fa-chevron-right'></i> 게시물관리 <i class='fa fa-chevron-right'></i> <span class='now'>상담사연관리</span> <i class='fa fa-chevron-right'></i> <span class='now'>게시물 상세</span></div>
						</div>
						...
						<div class="panel panel-default writeBox">
							<div class="panel-heading">게시물 상세</div>
						  	<div class="panel-body">
							  	<table class='table table-bordered' summary="상담사연상세">
							  		<caption style="display:none">상담 사연 상세</caption>
							  			<tbody>
							  				<tr>
							  	...
							  	<div class='text-right'>
							  		<c:if test="${CONSULT_AUTH eq '1' or sUserType eq '0'}">
   									    <a class='btn btn-primary btn-lg mr20 w100' href="/consult/updateView.do?confId=<%=jsonObj.get("CONF_ID") %>&boardNum=<%=jsonObj.get("BOARD_NUM") %>">수정</a>
	...
	function deleteBoard(){
		...
		if(result==true){
			location.href="/consult/deleteBoard.do?confId=<%=jsonObj.get("CONF_ID")%>&boardNum=<%=jsonObj.get("BOARD_NUM")%>";
		}else{
			return false
		}
	}
	</script>
<%-- 	<%if(CONSULT_AUTH.equals("1")||sUserType.equals("0")){ %>
		<script>
		//댓글리스트 ajax // 첫화면
		$.ajax({
		...
		<div id="boardReplyListView" class='contentArea'>
		</div>
		<%} %> --%>
</body>
</html>