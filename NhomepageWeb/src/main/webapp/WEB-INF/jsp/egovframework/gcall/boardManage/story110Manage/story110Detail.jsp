...
<title>110콜센터- 110이야기 관리 상세</title>
<script>
...
<body>
	<table class='layout' summary="110이야기 관리 상세 ">
		<%@include file="/WEB-INF/jsp/egovframework/gcall/index/top.jsp" %>
		<caption style="display:none">110이야기 관리 상세</caption>
		<tbody>
			<tr>
				<td scope="col" class='leftMenuArea'>
					...
					<td scope="col" class='contentArea'>
						...
						<div class='page-title'>
							...
							<div class='navi'><i class='fa fa-home'></i> <i class='fa fa-chevron-right'></i> 게시물관리 <i class='fa fa-chevron-right'></i> <span class='now'>110이야기 관리</span> <i class='fa fa-chevron-right'></i> <span class='now'>게시물 상세</span></div>
						</div>
						...
						<div class="panel panel-default writeBox">
							<div class="panel-heading">게시물 상세</div>
						  	<div class="panel-body">
							  	<table class='table table-bordered' summary="110이야기 관리 상세">
							  		<caption style="display:none">110이야기 관리 상세</caption>
							  			<tbody>
							  				<tr>
							  		...
							  		<div class='text-right'>
								  		<c:if test="${STORY110_AUTH eq '1' or sUserType eq '0'}">
								  			<a class='btn btn-primary btn-lg mr20 w100' href="/story110/updateView.do?confId=<%=jsonObj.get("CONF_ID") %>&boardNum=<%=jsonObj.get("BOARD_NUM") %>">수정</a>
										...
										<c:if test="${STORY110_AUTH ne '1' and sUserType ne '0'}">
	...
	function deleteBoard(){
		...
		if(result==true){
			location.href="/story110/deleteBoard.do?confId=<%=jsonObj.get("CONF_ID")%>&boardNum=<%=jsonObj.get("BOARD_NUM")%>";
		}else{
			return false
		}
	}
	</script>
<%-- 		<%if(STORY110_AUTH.equals("1")||sUserType.equals("0")){ %>
		<script>
		//댓글리스트 ajax // 첫화면
		$.ajax({
