...
//story110 110 news 임
@RequestMapping(value="/story110")
@Controller
public class Story110ManageController {
	...
	public ModelAndView list(
			@RequestParam(value = "confId", defaultValue = "11") String confId,
		...
		String story110Auth = CmmUtil.nvl((String) authUI.get("STORY110_AUTH"));
		String userType = CmmUtil.nvl((String) authUI.get("USER_TYPE"));
		if(story110Auth.equals("1")||userType.equals("0")) {
			...
			mv.setViewName("egovframework/gcall/boardManage/story110Manage/story110List");
	...
	public ModelAndView reg(HttpSession session) {
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			String userType = CmmUtil.nvl((String) authUI.get("USER_TYPE"));
			if(story110Auth.equals("1")||userType.equals("0")) {
				mv.setViewName("egovframework/gcall/boardManage/story110Manage/story110Reg");
			}else{
				mv.setViewName("egovframework/gcall/boardManage/redirect");
				mv.addObject("url","/g110/adm.do");
				mv.addObject("msg","권한이 없습니다.");
	...
	public ModelAndView regProc(BoardCommonDTO bDTO,
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			if(story110Auth.equals("1")||userType.equals("0")) {
		    	...
		    	mv.setViewName("redirect:/story110/list.do");
		    	bDTO=null;
			}else {
				mv.setViewName("egovframework/gcall/boardManage/redirect");
				mv.addObject("url","/g110/adm.do");
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			if(story110Auth.equals("1")||userType.equals("0")) {
				...
				for(int i=0; i<fList.size();i++) {
					File file = new File(req.getSession().getServletContext().getRealPath("")+fList.get(i).get("REAL_PATH")+""+EgovWebUtil.filePathBlackList(String.valueOf(fList.get(i).get("FILE_NAME"))));
				...
				mv.setViewName("egovframework/gcall/boardManage/story110Manage/story110Detail");
	...
	public ModelAndView updateView(HttpServletRequest req,HttpSession session) throws JSONException {
		...
		if(story110Auth.equals("1")||userType.equals("0")) {
			...
			JSONObject jsonObj = new JSONObject(hMap);
			JSONArray jsonFList = new JSONArray(fList);
			mv.setViewName("egovframework/gcall/boardManage/story110Manage/story110Update");
			mv.addObject("jsonObj", jsonObj);
			mv.addObject("fList", jsonFList);
			mv.addObject("newFList",newFList);
			hMap=null;
	...
	public ModelAndView deleteBoard(HttpServletRequest req,HttpSession session) {
		...
		if(hList!=null) {
			for(int i = 0 ; i < hList.size(); i++) {
				String filePathName = req.getSession().getServletContext().getRealPath("")+hList.get(i).get("REAL_PATH")+""+EgovWebUtil.filePathBlackList(String.valueOf(hList.get(i).get("FILE_NAME")));
		...
		mv.setViewName("egovframework/gcall/boardManage/redirect");
		mv.addObject("url","/story110/list.do");
