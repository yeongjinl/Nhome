...
@Controller
public class FreeBoardManageController {
	...
	public ModelAndView list(
			@RequestParam(value = "confId", defaultValue = "7") String confId,
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			String freeBoardAuth = CmmUtil.nvl((String) authUI.get("FREEBOARD_AUTH"));
			String userType = CmmUtil.nvl((String) authUI.get("USER_TYPE"));
			if(freeBoardAuth.equals("1")||userType.equals("0")) {
				...
				mv.setViewName("egovframework/gcall/boardManage/freeBoardManage/freeBoardList");
	...
	public ModelAndView reg(HttpSession session) {
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			String userType = CmmUtil.nvl((String) authUI.get("USER_TYPE"));
			if(freeBoardAuth.equals("1")||userType.equals("0")) {
				mv.setViewName("egovframework/gcall/boardManage/freeBoardManage/freeBoardReg");
			}else{
				mv.setViewName("egovframework/gcall/boardManage/redirect");
				mv.addObject("url","/g110/adm.do");
				mv.addObject("msg","권한이 없습니다.");
	...
	public ModelAndView regProc(BoardCommonDTO bDTO,
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			if(freeBoardAuth.equals("1")||userType.equals("0")) {
		    	...
		    	mv.setViewName("redirect:/freeBoard/list.do");
		    	bDTO=null;
			}else {
				mv.setViewName("egovframework/gcall/boardManage/redirect");
				mv.addObject("url","/g110/adm.do");
	...
	public ModelAndView detail(HttpServletRequest req,HttpSession session) throws Exception {
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			if(freeBoardAuth.equals("1")||userType.equals("0")) {
				...
				mv.setViewName("egovframework/gcall/boardManage/freeBoardManage/freeBoardDetail");
	...
	public ModelAndView updateView(HttpServletRequest req,HttpSession session) throws JSONException {
		...
		if(freeBoardAuth.equals("1")||userType.equals("0")) {
			...
			JSONObject jsonObj = new JSONObject(hMap);
			JSONArray jsonFList = new JSONArray(fList);
			mv.setViewName("egovframework/gcall/boardManage/freeBoardManage/freeBoardUpdate");
			mv.addObject("jsonObj", jsonObj);
			mv.addObject("fList", jsonFList);
			mv.addObject("newFList",newFList);
			hMap=null;
		...
		if(session.getAttribute("ss_user_info")!=null) {
			...
			if(freeBoardAuth.equals("1")||userType.equals("0")) {
				...
				if(FILE_NAME!=null) {
					...
					/*for(int i=0 ; i<rList.size();i++) {
						File f = new File(req.getSession().getServletContext().getRealPath("")+rList.get(i).get("FILE_NAME"));
						if(f.exists()) {
							f.delete();
						}
					}*/
