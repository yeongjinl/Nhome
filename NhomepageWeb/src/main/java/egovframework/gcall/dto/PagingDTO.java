...
public class PagingDTO {
	 ...
	 private int listSize;	// 게시판 끝 번호
	 private int rangeSize=5;	// 한 페이지 범위의 개수 (한 페이지에 출력되는 범위 개수) 10
	...
	public void pageInfo(int page, int listCnt) {
		...
		//전체 페이지수
		this.pageCnt = (int) Math.ceil(listCnt/(10*1.0));
		//현재 페이지 범위
		this.range = (int) Math.ceil(page/5.0);
		//시작 페이지
		this.startPage = (range - 1) * rangeSize + 1;
		//끝 페이지
		this.endPage = range * rangeSize;
		//게시판 시작 번호
		this.startList = ((page - 1) * 10) +1;
		//게시판 끝 번호
